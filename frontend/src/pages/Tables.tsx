...
export default function Tables() {
  ...
  const { data: tables = [], isLoading, error: tablesError } = useQuery<TableData[]>({
    queryKey: ['tables'],
    ...
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-on-surface-variant mt-0.5">{isLoading ? 'Loading…' : `${tables.length} table${tables.length !== 1 ? 's' : ''} configured`}</p>
        </div>
        <button
      ...
      {/* Table grid */}
      {tablesError ? (
        <div className="text-center py-20 text-error text-sm">Failed to load tables.</div>
      ) : isLoading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="bg-surface-container dark:bg-sumi-800 rounded-2xl border border-outline-variant/10 dark:border-sumi-700 p-4 animate-pulse">
              <div className="h-8 w-10 bg-surface-container-high rounded mb-3" />
              <div className="h-3 w-14 bg-surface-container-high rounded mb-3" />
              <div className="h-6 w-full bg-surface-container-high rounded mb-3" />
              <div className="h-7 w-full bg-surface-container-high rounded" />
            </div>
          ))}
        </div>
      ) : tables.length === 0 ? (
